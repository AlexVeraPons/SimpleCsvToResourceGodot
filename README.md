...
1.  Copy the `CSV_to_resource` folder into your project's `addons/` directory.
2.  Go to **Project > Project Settings > Plugins**.
3.  Enable **CSV to Resource**.
4.  The "CSV to Resource" dock will appear in Project>Tools>SimpleCsvToResourceGodot
...
*   **Gen Class**: Generates the `.gd` script file (based on the `class_name`).
*   **Gen Resources**: Generates the actual `.tres` resource files for each row in the CSV.
